...
func (r *LocateRepo) FindLTE(ctx context.Context, lte *model.LTE) (*model.Location, error) {
	const query = `
		SELECT lat, lon, from_metro, source
		FROM cells
...
func (r *LocateRepo) scanLocation(row *sql.Row) (*model.Location, error) {
	...
	if err := row.Scan(&loc.Point.Lat, &loc.Point.Lon, &loc.FromMetro, &source); err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			return nil, nil
		}
	...
	if source.Valid {
		loc.Source = model.Source(source.String)
	}
	return &loc, nil
}
...
func (r *LocateRepo) UpsertCell(row model.CellRefreshRow) error {
	...
	const query = `
		...
		DO UPDATE SET lat = EXCLUDED.lat, lon = EXCLUDED.lon,
		              from_metro = EXCLUDED.from_metro, source = EXCLUDED.source
	`
	_, err := r.db.Exec(query,
		...
		row.Lat, row.Lon, fromMetro, row.Source,
	)
	return err
}
